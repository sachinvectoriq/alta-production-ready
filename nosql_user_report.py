...
login_log_container = database.get_container_client('user_login_log')
...
def get_user_login_report():
    ...
    print(f"INFO: API endpoint to get user login report accessed by user: {user_name}")
    ...
    try:
        # Query user login data with optional filtering
        query = "SELECT * FROM c WHERE c.type = 'user_login'"
        ...
        if filter_user:
            query += f" AND c.user = '{filter_user}'"
        ...
        items = list(login_log_container.query_items(query=query, enable_cross_partition_query=True))

        # Get total count
        count_query = "SELECT VALUE COUNT(1) FROM c WHERE c.type = 'user_login'"
        if filter_user:
            count_query += f" AND c.user = '{filter_user}'"
        if domain_name:
            count_query += f" AND c.domain_name = '{domain_name}'"
        for excluded_user in EXCLUDED_USERS:
            count_query += f" AND c.user != '{excluded_user}'"

        total_count_result = list(login_log_container.query_items(query=count_query, enable_cross_partition_query=True))
