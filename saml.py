...
def saml_callback(saml_path):
    ...
    if not errors:
        ...
        # Extract name
        name = session['samlUserdata'].get(
            'http://schemas.microsoft.com/identity/claims/displayname', [''])[0]

        # Extract email with fallback
        email = session['samlUserdata'].get(
            'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress', [''])[0] \
            or session['samlUserdata'].get(
            'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name', [''])[0]
