...
# NOTE: "session" maps to login_session_id -- the DDL has no plain
# "session_id" column, only login_session_id. Flag if this is wrong.
order_dict = {
    "user": "user",
    "doc": "document_name",
...
def get_doc_data_report():
    ...
    try:
        ...
        if domain_name:
            ...
            params.append({"name": p, "value": domain_name})  # fixed: was .extend(), now correctly a single value
        ...
        # rename 'domain_name' -> 'opco' and 'login_session_id' -> 'session_id'
        # to match the original SELECT's column aliases
        for r in rows:
            if 'domain_name' in r:
                r['opco'] = r.pop('domain_name')
            if 'login_session_id' in r:
                r['session_id'] = r.pop('login_session_id')
