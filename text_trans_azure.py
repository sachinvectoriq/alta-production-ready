...
def fetch_settings(admin_id):
    """
    Point-read against the settings container -- admin_id is both the
    partition key and the document's Cosmos id, so this is a single,
    direct lookup with no query needed (same shape as the original
    single-row SELECT ... WHERE admin_id = %s).
    """
    try:
